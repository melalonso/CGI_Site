...
int main() {
    ...
    if (description.size() > 300) description = description.substr(0, 300);
    ...
    if (isUserLogged && isNumber(price)) {

        long priceInt = stol(price);
        if (trim(name) == "" || trim(description) == "") {
            cout << "Values could not be empty nor be spaces<br>";
            cout << "<a href='/cgi-bin/index'>Volver</a>";
            return 0;
        } else if (priceInt > 1000000 || priceInt <= 0) {
            cout << "Price could not be negative nor greater than 1000000<br>";
            cout << "<a href='/cgi-bin/index'>Volver</a>";
            return 0;
        } else {
        ...
        cout << "Could not insert product<br>";
    }
